...
static int kqemu_client_sync_dirty_bitmap(struct CPUPhysMemoryClient *client,
...
static int kqemu_client_migration_log(struct CPUPhysMemoryClient *client,
				      int enable)
{
	return 0;
}

static void kqemu_client_set_memory (struct CPUPhysMemoryClient *client,
                                     target_phys_addr_t start_addr, 
                                     ram_addr_t size, 
                                     ram_addr_t phys_offset);
static CPUPhysMemoryClient kqemu_cpu_phys_memory_client = {
	.set_memory = kqemu_client_set_memory,
	.sync_dirty_bitmap = kqemu_client_sync_dirty_bitmap,
	.migration_log = kqemu_client_migration_log,
};
...
int kqemu_init(CPUState *env)
{
    ...
    kinit.ram_dirty = ram_list.phys_dirty;
    ...
    qpi_init();
    // register a physical memory client
    cpu_register_phys_memory_client (&kqemu_cpu_phys_memory_client);
    return 0;
}
...
void kqemu_set_notdirty(CPUState *env, ram_addr_t ram_addr)
{
    ...
    if (ram_list.phys_dirty[ram_addr >> TARGET_PAGE_BITS] != 0xff)
        return;
...
static void kqemu_set_phys_mem(uint64_t start_addr, 
                               ram_addr_t size, 
                               ram_addr_t phys_offset)
{
